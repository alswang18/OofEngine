...
#define GFX_DEVICE_REMOVED_EXCEPT(hr)                                          \
  Graphics::DeviceRemovedException(                                            \
...
void
Graphics::ClearBuffer(float red, float green, float blue) noexcept
{
...
Graphics::DrawTestTriangle()
{
  ...
  // blob used for pixel and vertex shaders
  ...
  // We need vertex buffer information from pBlob so we define the input layout
  // after the vertex shader is created
  GFX_THROW_INFO(pDevice->CreateInputLayout(ied,
                                            (UINT)std::size(ied),
